...
Откройте раздел **Ресурс-паки** и нажмите **«Искать»**: с пустым полем откроется популярный каталог. Для конкретного набора введите запрос. Лаунчер сначала получает только проекты типа `resourcepack` с точной совместимостью с активной Vanilla-версией. Если поисковый индекс Modrinth ещё не отметил свежую версию Minecraft, показывается резервный каталог с явной пометкой **«проверка при установке»**. Перед загрузкой лаунчер всегда повторно запрашивает версии выбранного проекта и разрешает установку лишь при точном совпадении версии игры. Сетевые ошибки Modrinth, HTTP-статус и описание ответа выводятся в интерфейсе; запрос автоматически повторяется один раз.
...
python scripts_smoke_modrinth_v2.py
python scripts_install_modrinth_smoke.py
```

Они проверяют получение всех Vanilla-версий из manifest, актуальный релиз, поиск Modrinth, резервный каталог, выбор совместимого файла, скачивание с SHA-1-проверкой и декодирование PNG-иконки resource pack.
